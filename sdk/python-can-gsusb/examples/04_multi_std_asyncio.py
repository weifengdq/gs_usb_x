...
async def periodic_sender(channel_index: int, bus: GsUsbBus, stats, period_s: float, use_fd: bool, stop_event: asyncio.Event) -> None:
    ...
    try:
        while not stop_event.is_set():
            payload = [channel_index, counter & 0xFF, (counter >> 8) & 0xFF, 0x31, 0x41, 0x59, 0x26, 0x53]
            if use_fd:
                payload += [0x58, 0x97, 0x93, 0x23]
            msg = can.Message(
                arbitration_id=0x300 + channel_index,
...
async def main_async(args) -> None:
    ...
    try:
        ...
        print(f"Running 4-channel asyncio demo for {run_duration_s:.1f}s...")
        ...
        print_stats_summary("4-channel asyncio summary", CHANNELS, stats, elapsed_s)
        for bus in buses:
            bus.shutdown()
