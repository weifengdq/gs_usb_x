...
int main(int argc, char** argv) {
    try {
        const auto options = demo::parse_common_options(argc, argv, "4-Channel Custom Timing Async Example");
        ...
        return demo::run_multi_summary_demo(
            "Running 4-channel custom timing async demo for " + std::to_string(options.duration_s) + "s...",
            ...
            [&](int channel_index, std::uint64_t counter) {
                ...
                msg.arbitration_id = 0x800u + static_cast<std::uint32_t>(channel_index);
                msg.is_fd = true;
