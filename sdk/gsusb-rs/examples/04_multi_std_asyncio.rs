...
fn main() -> Result<()> {
    ...
    let options = parse_common_options(&args, "4-channel standard CAN async example")?;
    ...
    run_multi_summary_demo(
        &format!("Running 4-channel async demo for {:.6}s...", options.duration_s),
        ...
        |channel, counter| CanMessage {
            arbitration_id: 0x300 + u32::from(channel),
            is_fd: use_fd,
            bitrate_switch: use_fd,
            data: make_payload(&[channel, (counter & 0xFF) as u8, ((counter >> 8) & 0xFF) as u8, 0x31, 0x41, 0x59, 0x26, 0x53], if use_fd { 12 } else { 8 }, 0x58),
            ..CanMessage::default()
        },
    )?;
    Ok(())
