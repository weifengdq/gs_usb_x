...
fn main() -> Result<()> {
    ...
    let options = parse_common_options(&args, "4-channel custom timing threading example")?;
    ...
    run_multi_summary_demo(
        &format!("Running 4-channel custom timing threading demo for {:.6}s...", options.duration_s),
        ...
        |channel, counter| CanMessage {
            arbitration_id: 0x700 + u32::from(channel),
            is_fd: true,
            bitrate_switch: true,
