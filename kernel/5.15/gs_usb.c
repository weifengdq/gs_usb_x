...
#define DECLARE_FLEX_ARRAY(_type, _name) _type _name[]
#endif
...
static inline int can_rx_offload_queue_timestamp(struct can_rx_offload *offload,
					    ...
					    unsigned int *frame_len_ptr)
{
	...
	return can_rx_offload_get_echo_skb(offload, idx, timestamp,
						      frame_len_ptr);
}

static inline unsigned int
can_rx_offload_get_echo_skb_queue_tail(struct can_rx_offload *offload,
				      unsigned int idx, unsigned int *frame_len_ptr)
{
	unsigned int frame_len;
	...
	return can_rx_offload_get_echo_skb(offload, idx, 0, frame_len_ptr);
}
#endif
...
static inline int can_eth_ioctl_hwts(struct net_device *netdev,
...
static inline int
can_ethtool_op_get_ts_info_hwts(struct net_device *netdev,
